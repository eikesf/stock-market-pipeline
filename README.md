...
│   ├── archive/             # Landing files already ingested (prices/ & metadata/); replay log for Bronze recovery
...
│   │   ├── utils.py         # Shared IO utilities (Delta read/write, ClickHouse & corruption recovery)
│   │   ├── quality_rules.py # Row-level quality rules and valid/quarantine split
...
4.  **`dag_delta_maintenance`** (Weekly Maintenance Pipeline):
    ...
    *   **Flow:** Verifies Delta integrity (`task_verify_delta_integrity`) → runs compaction (`OPTIMIZE`) and cleanup (`VACUUM`) (`task_optimize_and_vacuum`) on every Bronze and Silver Delta table, including the quarantine tables, to resolve the small file problem and manage storage.
    *   **Integrity verification:** reads every data file in every table to surface physically corrupt parquet before it blocks a pipeline. Nothing else notices a damaged file until a query happens to touch it, so this is what turns a silent problem into a same-week alert. Each table is maintained independently, so one corrupt table no longer skips compaction for the rest.
...
### Delta Corruption Detection & Recovery

Because the Delta tables live on a local Docker bind mount, a lost write can leave a physically damaged Parquet file that only surfaces the next time a query happens to read it. The pipeline therefore both looks for damage on a schedule and repairs it when a task hits it (`src/streaming/utils.py`):

*   **Proactive detection**: `task_verify_delta_integrity` (in `dag_delta_maintenance`) reads every row group of every data file weekly, since a file's footer can be intact while its data pages are not.
*   **Reactive healing**: every layer's `except` block runs `check_and_heal_corrupt_data_file`, which finds the Delta version that introduced the damaged file, `RESTORE`s the table to the version before it, and deletes the physical file. Checkpoint corruption is repaired separately on every read/write by `heal_corrupt_delta_checkpoints`.
*   **Bronze archive replay**: a rollback also discards the good commits made after the damaged one. Silver and Gold can simply be recomputed from upstream, but Bronze holds the only copy of the ingested rows — so a Bronze rollback is followed by `recover_bronze_from_archive`, which reads the highest date still present in the restored table and replays only the archived landing files past it (rather than the whole archive). Overlapping replays are harmless because Silver deduplicates on `(ticker, date)` keeping the latest `ingestion_timestamp`.
*   **Alerting is not suppressed**: healing prevents *permanent* corruption, not the current run's failure. A successful heal still raises, so the task fails and the email/Discord alerts fire — the run is then safe to retry.
