...
*   **Outlier Quarantine (pre-scan sanitization)**: Before the Silver metrics Soda scan runs, any ticker row with an implausible `trailing_eps`, `price_to_sales`, or `operating_margins` value (e.g. a negative price-to-sales ratio) is moved whole out of `silver_metrics` into a separate `silver_metrics_rejected` Delta table, keeping its original values plus a `failed_checks` list and `rejected_at` timestamp for manual review. `silver_metrics` therefore only holds rows that passed every check, and one bad ticker no longer fails the entire batch's quality scan.
