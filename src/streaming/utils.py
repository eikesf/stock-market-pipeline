...
def ingest_landing_to_bronze(  # noqa: PLR0913
    ...
    matching_files = list(landing_dir.glob(file_pattern))
    if matching_files:
        landing_file = matching_files[0]
        in_landing = True
    else:
        landing_file = landing_dir / archive_name_format
        in_landing = False

    # Check if the file has already been archived (processed by another DAG)
    if not in_landing or not landing_file.exists():
        archive_file = archive_dir / archive_name_format
        matching_archive_files = list(archive_dir.glob(file_pattern))
    ...
    try:
        ...
        if landing_file.exists():
            dest_file = archive_dir / landing_file.name
            if dest_file.exists():
                dest_file.unlink()
