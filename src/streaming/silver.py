...
logger.info("Starting Silver layer processing for stock prices...")
...
try:
    ...
    stock_df_silver = stock_df_silver.withColumn("rn", row_number().over(window_spec)).filter(col("rn") == 1).drop("rn")
